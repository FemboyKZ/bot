...
module.exports = {
    ...
    async execute(interaction) {
        ...
        switch (sub) {
            case 'add':
                if (data) {
                    return await interaction.reply({ content: `You already have this reaction setup, using ${emoji}, on this message.`, ephemeral: true, })
                } else {
                    ...
                    const embed = new EmbedBuilder()
                        .setColor("#ff00b3")
                        .setDescription(`A Reaction role has been added to ${message.url} with ${emoji} and the role ${role}`)
            ...
            case 'remove':
                if (!data) {
                    return await interaction.reply({ content: `That reaction role doesn't exist`, ephemeral: true, })
                } else {
                    ...
                    const embed = new EmbedBuilder()
                        .setColor("#ff00b3")
                        .setDescription(`A Reaction role has been removed from ${message.url} with ${emoji}`)
